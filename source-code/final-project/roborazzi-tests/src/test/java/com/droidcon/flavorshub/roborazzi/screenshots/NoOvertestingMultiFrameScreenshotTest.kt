 ...
 * Record:
 * ./gradlew :roborazzi-tests:recordRoborazziDebug --tests 'NoOvertestingMultiFrameScreenshotTest'
 * Verify:
 * ./gradlew :roborazzi-tests:verifyRoborazziDebug --tests 'NoOvertestingMultiFrameScreenshotTest'
 */
@HiltAndroidTest
@Config(application = HiltTestApplication::class)
@RunWith(RobolectricTestParameterInjector::class)
@GraphicsMode(GraphicsMode.Mode.NATIVE)
class NoOvertestingMultiFrameScreenshotTest(
    ...
    val robolectricQualifiersRule = object : TestWatcher() {
        override fun starting(description: Description) {
            setQualifiers(testCase.device.qualifier)
            setQualifiers(testCase.uiMode.cumulativeQualifier)
            setQualifiers(testCase.locale.cumulativeQualifier)
            setFontScale(testCase.fontSize.scale)
        }
    }
